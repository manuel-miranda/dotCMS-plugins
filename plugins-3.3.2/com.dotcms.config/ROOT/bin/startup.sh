...
if [ "$EUID" != "8080" ]; then
        echo "This startup script can only be run as the 'dotcms' user"
        exit
fi
...
if [ -e /opt/dotcms/logs/tomcat/gc.log ]; then
  mv /opt/dotcms//logs/tomcat/gc.log /opt/dotcms/logs/tomcat/gc.log.$TIMESTAMP
fi
...
JAVA_OPTS="$JAVA_OPTS -Djava.awt.headless=true -Xverify:none -Dfile.encoding=UTF8 -server -Xmx10G -XX:MaxMetaspaceSize=512m -XX:+DisableExplicitGC -XX:+UseG1GC -javaagent:$DOTCMS_HOME/WEB-INF/lib/dot.jamm-0.2.5_2.jar"
