...
## BEGIN DOTCMS CLOUD CUSTOMIZATIONS

if [ "$EUID" != "8080" ]; then
        echo "This script can only be run as the 'dotcms' user"
        exit
fi

# This is needed for dotcms operating environment
#export JAVA_HOME=/opt/dotcms/java
...
if [ -e $CATALINA_PID ]; then
  echo "System Load: $(cat /proc/loadavg)" >/opt/dotcms/logs/tomcat/thread_cpu_$(hostname -s)_$TIMESTAMP.txt
  echo "Thread CPU usage for PID $(cat $CATALINA_PID)" >>/opt/dotcms/logs/tomcat/thread_cpu_$(hostname -s)_$TIMESTAMP.txt
  echo "  PID   TID NLWP %CPU     TIME PSR %MEM" >>/opt/dotcms/logs/tomcat/thread_cpu_$(hostname -s)_$TIMESTAMP.txt
  ps --pid $(cat $CATALINA_PID) -L -o pid,tid,nlwp,pcpu,cputime,psr,pmem  |grep ^[0-9] |sort -rn -k 4 >>/opt/dotcms/logs/tomcat/thread_cpu_$(hostname -s)_$TIMESTAMP.txt
  $JAVA_HOME/bin/jstack -l $(cat $CATALINA_PID) > /opt/dotcms/logs/tomcat/threaddump_$(hostname -s)_$TIMESTAMP.txt
  sleep 1
fi

## END DOTCMS CLOUD CUSTOMIZATIONS
...
if [ -z "$CATALINA_PID" ]; then
        ...
        if [ "$1" = "start" ] ; then
                if [ -e "$CATALINA_PID" ]; then
                        ...
                        echo
                        echo "If dotCMS is not running, please remove the Pid file or change the"
                        echo "setting in bin/catalina.sh before starting your dotCMS application."
                        echo
                        exit 1
                fi
        fi
...
MEMSET=`echo $JAVA_OPTS | grep '\-XX:MetaspaceSize'`
if [ -z "$MEMSET" ]; then
    export JAVA_OPTS="$JAVA_OPTS -XX:MetaspaceSize=196m"
fi
